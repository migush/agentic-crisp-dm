...
| Loop | Trigger | Action |
|---|---|---|
| A 2→1 | Non-empty quality **blockers** (not documented `high_missing` / `na_means_absent` alone) or domain `loop_a_recommendation.should_trigger` | Return to **1.3** |
...
Hard caps (phase visits, Loop B iterations, Loop C once, Loop A without blockers) are
enforced mechanically — if a loop cannot fire, advance the current phase rather than
aborting the run.
