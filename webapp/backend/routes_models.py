"""Authenticated proxy for live chat-model lists (OpenAI and Ollama Cloud)."""
...
from .hosted import list_live_chat_models
...
def list_models(body: ListModelsRequest, _user_id: int = Depends(require_user)) -> list[dict[str, str]]:
    ...
    return list_live_chat_models(body.provider, body.decrypted_api_key)
